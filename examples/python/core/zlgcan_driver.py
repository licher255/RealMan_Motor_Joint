...
class ZlgCanDriver:
    ...
    def receive_frame(self, timeout_ms: int = 100) -> Optional[CANFDFrame]:
        """
    ...
    def receive(self, timeout_ms: int = 100) -> Optional[CANFDFrame]:
        """Alias for receive_frame"""
        return self.receive_frame(timeout_ms)

    def receive_all(self, max_frames: int = 100) -> List[CANFDFrame]:
        """
        ...
        for _ in range(max_frames):
            frame = self.receive_frame(timeout_ms=0)
            if frame is None:
                break
            frames.append(frame)
        return frames
...
if __name__ == "__main__":
    ...
    try:
        ...
        # Send test frame
        print("\n[Test] Sending test frame...")
        test_data = bytes([0x01, 0x10, 0x00, 0x01])  # Read register command
        success = driver.send(can_id=0x01, data=test_data)
        print(f"[Test] Send result: {'OK' if success else 'FAILED'}")
        
        # Receive frames
        print("\n[Test] Receiving frames (press Ctrl+C to stop)...")
        try:
            while True:
